...
def main_render():
    ...
    # Parse time
    trial_t = current_t % conf.design.trial.interval
    trial_idx = current_t // conf.design.trial.interval
    cue_idx = conf.design.cueSequence.sequence[int(trial_idx % len(
        conf.design.cueSequence.sequence))]

    # Create state
    state = tt.check(trial_t)

    # aspect
    aspect = wnd.width / wnd.height

    # --------------------
    # Draw the texts
    for i, obj in conf.design.texts.items():
        text, x, y, w, h = obj

        if state is TrialState.Cue and i == cue_idx:
            c = conf.design.color.cueStroke
        else:
            c = conf.design.color.textBackground
        wnd.draw_rect(x, y, w, h, c)

        c = conf.design.color.textBackground
        padding_x = w*0.1 / aspect
        padding_y = h*0.1
        wnd.draw_rect(x+padding_x, y+padding_y, w -
                      2*padding_x, h-2*padding_y, c)

        c = conf.design.color.textColor
        if '\n' in text:
            t1, t2 = text.split('\n', 1)
            wnd.draw_text(t1, x+w/2, y+h/2, scale=1,
                          anchor=TextAnchor.B, color=c)
            wnd.draw_text(t2, x+w/2, y+h/2, scale=1,
                          anchor=TextAnchor.T, color=c)
        else:
            wnd.draw_text(text, x+w/2, y+h/2, scale=1,
                          anchor=TextAnchor.C, color=c)

    # --------------------
    # Draw the blocks
    for obj in conf.design.blocks.values():
        f, x, y, w, h = obj

        if state is TrialState.Blinking:
            c = (math.sin(current_t*2*math.pi*f) + 1) * 0.5
        else:
            c = 0.5
    ...
    # --------------------
    # Draw the cursor texts
    x, y = wnd.cursor_pos
    x /= wnd.width
    y /= wnd.height
    y = 1-y
    text = f'{current_t: 0.3f} | {trial_t:0.3f}'
    wnd.draw_text(text, x, y, scale=1,
...
argparse.add_argument('--conf', type=str, default='./conf/experiment1.yaml',
                      help='The path of the conf file')
...
# %%
conf = OmegaConf.load(opt.conf)
logger.info(f'Using {conf=}')


# %%
class TrialTime:
    interval = conf.design.trial.interval
    cue = conf.design.trial.cue
    blinking = conf.design.trial.blinking

    def check(self, t):
        if t > self.interval - self.blinking:
            return TrialState.Blinking

        if t > self.interval - self.blinking - self.cue:
            return TrialState.Cue
...
wnd.load_font('./resource/font/MTCORSVA.TTF', FONT_SIZE)
wnd.init_window()
