...
Purpose:
    The basic demo with cursor interaction.
    The rectangle under cursor is flipped at given frequency.
...
def main_render(freq=0.2):
    # Get time
    t = glfw.get_time()

    # Draw the blocks
    for block in conf.design.blocks.values():
        f, x, y, w, h = block
    ...
    # Draw the cursor blinking block
    omega = freq * 2 * math.pi
    c = (math.sin(t*omega) + 1) * 0.5
    # c *= 0.5
    face_color = (c, c, c, 0.8)
    ...
    text = f'{freq=:0.2f} Hz'
    wnd.draw_text(text, x/2, y, scale=1,
                  anchor=TextAnchor.B, color=PROMPT_COLOR)
...
argparse.add_argument('--freq', type=float, default=0.2,
                      help='The blinking frequency of the square.')
...
# Run FOREVER
wnd.render_loop(main_render, freq=opt.freq)
